...
public class CowKiller extends Script {
    ...
    private final Coordinate bankLocation = new Coordinate(2889, 3537, 0); // Example coordinates for the bank in Burthorpe
    ...
    public void run() {
        ...
        if (LocalPlayer.self().getTargetType() != EntityType.NPC_ENTITY && COW_AREA.contains(LocalPlayer.self())) {
            ...
            cows.stream()
                .filter(npc -> npc.getName().equalsIgnoreCase("Cow") || npc.getName().equalsIgnoreCase("Super cow"))
                .filter(npc -> COW_AREA.contains(npc))
                .findFirst()
                .ifPresent(npc -> {
                    println("Found cow: " + npc.getName());
                    npc.interact("Attack");
                });
        } else {
            println("Already attacking a cow or no cows found in the area.");
